      ...
      <Card className="p-6 bg-gradient-card">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="p-4 rounded-xl bg-success/10">
            ...
            <p className="text-2xl font-bold text-success">{formatCurrency(quickStats.ingresos)}</p>
          </div>
          <div className="p-4 rounded-xl bg-destructive/10">
            <p className="text-sm text-muted-foreground">Gastos</p>
            <p className="text-2xl font-bold text-destructive">{formatCurrency(quickStats.gastos)}</p>
          </div>
          <div className="p-4 rounded-xl bg-primary/10">
            <p className="text-sm text-muted-foreground">Resultado</p>
            <p className="text-2xl font-bold text-primary">{formatCurrency(quickStats.resultado)}</p>
          </div>
          <div className="p-4 rounded-xl bg-accent/10">
            <p className="text-sm text-muted-foreground">Rentabilidad</p>
            <p className="text-2xl font-bold text-accent">{quickStats.rentabilidad}%</p>
          </div>
        </div>
      </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <Textarea placeholder="Describe el informe que necesitas..." value={mensaje} onChange={(e) => setMensaje(e.target.value)} />
            {confirmacion && <p className="text-sm text-center">{confirmacion}</p>}
          </div>
          <DialogFooter>
      ...
      <Dialog open={openPeriodo} onOpenChange={setOpenPeriodo}>
        <DialogContent className="max-w-sm">
          ...
          <div className="flex flex-col gap-3">
            <select className="border rounded-lg p-2" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)}>
              {months.map((m) => (
      ...
      <Dialog open={openPreview} onOpenChange={setOpenPreview}>
        <DialogContent>
          ...
          <div className="space-y-3 text-sm text-muted-foreground">
            <p><strong>Período:</strong> {selectedMonth} {selectedYear}</p>
            <p><strong>Ingresos:</strong> {formatCurrency(quickStats.ingresos)}</p>
            <p><strong>Gastos:</strong> {formatCurrency(quickStats.gastos)}</p>
            <p><strong>Resultado:</strong> {formatCurrency(quickStats.resultado)}</p>
            <p><strong>Rentabilidad:</strong> {quickStats.rentabilidad}%</p>
          </div>
          <DialogFooter>
